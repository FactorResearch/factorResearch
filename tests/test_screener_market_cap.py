...
class TestEnrichFromAnalysisCacheMarketCap:
    ...
    def test_uses_top_level_market_cap_when_present(self):
        ...
        with patch("codes.engine.screener.cache.list_cached_kind", return_value=["MSFT"]), \
             patch("codes.engine.screener.cache.read", return_value=cached):
    ...
    def test_new_symbol_appended_with_market_cap(self):
        ...
        with patch("codes.engine.screener.cache.list_cached_kind", return_value=["TSLA"]), \
             patch("codes.engine.screener.cache.read", return_value=cached):
            screener._enrich_from_analysis_cache()
