...
def test_dated_direct_link_ticker_drives_analysis_when_input_is_empty():
    ...
    analyze_stock.assert_called_once_with("AAPL", force_refresh=True)
    assert re.fullmatch(r"/AAPL/analyze/\d{8}", output[0])
    assert output[1].to_plotly_json()["props"]["children"] == ["content"]
    assert output[6] == "AAPL"
    assert output[7] == {"display": "block"}
    assert output[8] == "AAPL"


def test_screener_full_analysis_returns_one_callback_root():
    result = {"name": "Apple Inc.", "cache_hit": True, "cache_source": "database"}

    with patch.object(analyze_tab.dash, "ctx", SimpleNamespace(triggered_id="screener-open-analysis-symbol")), \
         patch.object(analyze_tab, "check_rate_limit"), \
         patch.object(analyze_tab, "get_user_id", return_value=None), \
         patch.object(
             analyze_tab.permissions,
         ...
         patch.object(analyze_tab.product_analytics, "track_event"), \
         patch.object(analyze_tab, "analyze_stock", return_value=result), \
         patch.object(analyze_tab, "_build_analysis_content", return_value=["content"]), \
         patch.object(analyze_tab.screener, "update_stock_after_analysis"):
        output = analyze_tab.run_analysis(None, "AAPL", "/", None, [])

    serialized = output[1].to_plotly_json()
    assert serialized["props"]["className"] == "analysis-callback-root"
    assert serialized["props"]["children"] == ["content"]
