"""UK market provider adapter.

This module is intentionally source-injected. Production can plug in verified
Companies House, FCA NSM, issuer-report, or licensed feeds without coupling
analysis engines to a source payload or scraping public web pages.
"""
...
class UKDataSource(Protocol):
    """Source interface for UK issuer data."""
...
class EmptyUKDataSource:
    """No-network default source used until a UK feed is configured."""
...
class UKProviderAdapter:
    """Normalize UK issuer data into provider-neutral objects."""
    ...
    def __init__(self, source: UKDataSource | None = None) -> None:
        self.source = source or EmptyUKDataSource()

    def get_company(self, symbol: str) -> CanonicalCompany:
        symbol = normalize_uk_symbol(symbol)
        ...
        return CanonicalCompany(
            ...
            regulator_id=profile.get("regulator_id") or profile.get("company_number"),
    ...
    def get_filings(self, symbol: str) -> list[dict[str, Any]]:
        return list(self.source.get_filings(normalize_uk_symbol(symbol)))
    ...
    def get_source_documents(self, symbol: str) -> list[FilingDocument]:
        ...
        rows = getter(normalize_uk_symbol(symbol)) if getter else []
    ...
    def get_currency_info(self, symbol: str) -> CanonicalCurrency:
        ...
        return CanonicalCurrency(code=code, display_name="British pound" if code == "GBP" else None)
...
def normalize_uk_symbol(symbol: str) -> str:
    """Normalize common UK ticker input without guessing listings."""
    value = str(symbol or "").upper().strip()
    if not value:
        return value
    value = value.replace(":", ".")
    if value.endswith(".LSE"):
        return value[:-4] + ".L"
    return value


def is_uk_symbol(symbol: str) -> bool:
    value = normalize_uk_symbol(symbol)
    return value.endswith(".L") and len(value) > 2
