...
def _legacy_factor_research(data: dict) -> dict:
    ...
    if capm.get("error"):
        return {"status": "data_unavailable", "message": "Needs more benchmark history."}
    return {"status": "ready", "model": "capm", "models": {"capm": capm}, "source": "saved histories"}


def _factor_research_card(data: dict) -> html.Div:
    research = data.get("factor_research") or _legacy_factor_research(data)
    models = research.get("models") or {}
    labels = {"capm": "CAPM", "ff3": "Fama-French 3", "ff5": "Fama-French 5", "carhart4": "Carhart 4"}
    if not models:
        return _metric_scorecard(
            title="Factor Research", status_text="Pending", status_color=AMBER,
            subtitle=research.get("message", "Waiting for return history."),
        )
    primary = research.get("model") or next(iter(models))
    result = models.get(primary) or {}
    beta = (result.get("betas") or {}).get("mkt_rf")
    color = GREEN if beta is not None and beta < 1 else AMBER if beta is not None and beta <= 1.3 else RED
    rows = []
    for key in ("capm", "ff3", "ff5", "carhart4"):
        model = models.get(key)
        if not model:
            continue
        model_beta = (model.get("betas") or {}).get("mkt_rf")
        value = (f"beta {model_beta:.2f} / alpha {(model.get('alpha_annualized') or 0) * 100:.1f}%"
                 if model_beta is not None else model.get("error", "Unavailable"))
        rows.append(_metric_data_row(labels[key], value))
    attribution = research.get("return_attribution") or result.get("return_attribution") or {}
    if attribution.get("total_excess_return") is not None:
        rows.append(_metric_data_row("Attributed excess return", f"{attribution['total_excess_return'] * 100:.1f}%"))
    rows.append(_metric_data_row("Source", research.get("source", "saved analysis")))
    return _metric_scorecard(
        title="Factor Research", score_text=f"{beta:.2f}" if beta is not None else "N/A",
        score_color=color, status_text=labels.get(primary, primary), subtitle="V2.2 factor attribution",
        body_children=rows, card_class="factor-research-card",
    )
    ...
    sections = [
        ...
        (
            ...
            _section(
                ...
                [
                    html.Div(className="analysis-card-grid analysis-card-grid--two", children=[risk_card, factor_research_card]),
                    html.Div(
