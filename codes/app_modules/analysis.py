...
def _analyze_stock(symbol: str, *, force_refresh: bool = False, defer_secondary: bool = False) -> dict:
    ...
    with _analysis_cache_lock:
        cached_memory = None if force_refresh else _analysis_cache.get(symbol)
    if cached_memory:
        cached_memory = enrich_cached_analysis_if_needed(symbol, cached_memory)
    ...
    beneish_result = _timed(timings, "beneish", lambda: beneish.score(sec_facts))
    dechow_result = _timed(timings, "dechow", lambda: dechow.score(sec_facts))
    ...
    accounting_quality_result = _optional_component(
        timings, "accounting_quality", symbol, sec_facts,
        lambda: accounting_quality_model.AccountingQualityAnalyzer(
            symbol,
            sec_facts,
