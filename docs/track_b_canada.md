...
These files are not generated by the worker and are not bundled with the
repository. They must be produced first by a verified SEDAR+/issuer/licensed
source extraction process. The worker validates, normalizes, quality-gates and
stores that existing export; it is not a filing downloader.
