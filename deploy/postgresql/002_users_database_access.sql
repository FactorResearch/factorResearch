-- ISSUE_082 users-database connection and schema ownership boundary.
-- Run as the database owner while connected to the users database, after the
-- canonical cluster roles exist and before the release migration command.

DO $issue_082_users_database$
DECLARE
    relation RECORD;
BEGIN
    EXECUTE format('REVOKE CONNECT, TEMPORARY ON DATABASE %I FROM PUBLIC', current_database());
    EXECUTE format(
        'GRANT CONNECT ON DATABASE %I TO cenvarn_app, cenvarn_service, '
        'cenvarn_migration, cenvarn_readonly',
        current_database()
    );
    EXECUTE format(
        'REVOKE CONNECT ON DATABASE %I FROM cenvarn_market_worker',
        current_database()
    );
    FOR relation IN
    ...
    END LOOP;
END
$issue_082_users_database$;

REVOKE ALL ON SCHEMA public FROM PUBLIC;
ALTER SCHEMA public OWNER TO cenvarn_migration;
GRANT USAGE ON SCHEMA public TO cenvarn_app, cenvarn_service, cenvarn_readonly;
